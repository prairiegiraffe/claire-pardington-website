...
interface ThreeLogosFeatureComparisonPricingProps {
  title?: string;
  subtitle?: string;
  tiers?: Tier[];
  sections?: Section[];
...
const defaultLogos = [
  ...
  { name: 'Laravel', src: 'https://tailwindcss.com/plus-assets/img/logos/158x48/laravel-logo-white.svg' },
              ...
              {tiers.map((tier) => (
                <td key={tier.name} className="px-0 pt-3 pb-0">
                  <a
                    ...
                    className="inline-block rounded-md bg-white px-2.5 py-1.5 text-sm font-semibold text-gray-900 shadow-xs ring-1 ring-inset ring-gray-300 hover:bg-gray-50 dark:bg-white/10 dark:text-white dark:shadow-none dark:ring dark:ring-white/5 dark:hover:bg-white/20"
                  >
                    Get started
                  </a>
                </td>
        ...
        <TabGroup className="sm:hidden">
          <TabList className="flex">
            {tiers.map((tier) => (
              <Tab
                key={tier.name}
                className="w-1/3 border-b border-gray-100 py-4 text-base/8 font-medium text-indigo-600 focus:outline-none data-[selected]:border-indigo-600 dark:border-white/10 dark:text-indigo-400 dark:data-[selected]:border-indigo-400"
              >
                {tier.name}
              </Tab>
            ))}
          ...
          <TabPanels as={Fragment}>
            {tiers.map((tier) => (
              <TabPanel key={tier.name} className="focus:outline-none">
                <a
                  href={tier.href}
                  className="mt-8 block rounded-md bg-white px-3.5 py-2.5 text-center text-sm font-semibold text-gray-900 shadow-xs ring-1 ring-inset ring-gray-300 hover:bg-gray-50 dark:bg-white/10 dark:text-white dark:shadow-none dark:ring-transparent dark:ring-white/5 dark:hover:bg-white/20"
                >
                  Get started
                </a>
                {sections.map((section) => (
